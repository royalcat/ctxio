...
func TestCopy(t *testing.T) {
	...
	Copy(ctx, WrapIoWriter(wb), WrapIoReader(rb))
...
func TestCopyNegative(t *testing.T) {
	...
	Copy(ctx, WrapIoWriter(wb), &LimitedReader{R: WrapIoReader(rb), N: -1})
	if wb.String() != "" {
		t.Errorf("Copy on LimitedReader with N<0 copied data")
	}

	CopyN(ctx, WrapIoWriter(wb), WrapIoReader(rb), -1)
	if wb.String() != "" {
...
func TestCopyBuffer(t *testing.T) {
	...
	wb := new(Buffer)
	rb.WriteString("hello, world.")
	CopyBuffer(ctx, WrapIoWriter(wb), WrapIoReader(rb), make([]byte, 1)) // Tiny buffer to keep it honest.
	if wb.String() != "hello, world." {
...
func TestCopyBufferNil(t *testing.T) {
	...
	wb := new(Buffer)
	rb.WriteString("hello, world.")
	CopyBuffer(ctx, WrapIoWriter(wb), WrapIoReader(rb), nil) // Should allocate a buffer.
	if wb.String() != "hello, world." {
		t.Errorf("CopyBuffer did not work properly")
	}
}
...
func TestCopyReadErrWriteErr(t *testing.T) {
	...
	n, err := Copy(ctx, WrapIoWriter(w), WrapIoReader(r))
	if n != 0 || err != ew {
...
func TestCopyN(t *testing.T) {
	...
	wb := new(Buffer)
	rb.WriteString("hello, world.")
	CopyN(ctx, WrapIoWriter(wb), WrapIoReader(rb), 5)
	if wb.String() != "hello" {
...
func BenchmarkCopyNSmall(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		CopyN(ctx, WrapIoWriter(buf), WrapIoReader(rd), 512)
		rd.Reset(bs)
	}
}
...
func BenchmarkCopyNLarge(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		CopyN(ctx, WrapIoWriter(buf), WrapIoReader(rd), 32*1024)
		rd.Reset(bs)
	}
}
...
func (w *noReadFrom) Write(ctx context.Context, p []byte) (n int, err error) {
	return w.w.Write(ctx, p)
}

type wantedAndErrReader struct{}

func (wantedAndErrReader) Read(ctx context.Context, p []byte) (int, error) {
...
func TestCopyNEOF(t *testing.T) {
	...
	n, err := CopyN(ctx, &noReadFrom{WrapIoWriter(b)}, WrapIoReader(strings.NewReader("foo")), 3)
	if n != 3 || err != nil {
		t.Errorf("CopyN(noReadFrom, foo, 3) = %d, %v; want 3, nil", n, err)
	}

	n, err = CopyN(ctx, &noReadFrom{WrapIoWriter(b)}, WrapIoReader(strings.NewReader("foo")), 4)
	if n != 3 || err != EOF {
		t.Errorf("CopyN(noReadFrom, foo, 4) = %d, %v; want 3, EOF", n, err)
	}

	n, err = CopyN(ctx, WrapIoWriter(b), WrapIoReader(strings.NewReader("foo")), 3) // b has read from
	if n != 3 || err != nil {
		t.Errorf("CopyN(bytes.Buffer, foo, 3) = %d, %v; want 3, nil", n, err)
	}

	n, err = CopyN(ctx, WrapIoWriter(b), WrapIoReader(strings.NewReader("foo")), 4) // b has read from
	if n != 3 || err != EOF {
		t.Errorf("CopyN(bytes.Buffer, foo, 4) = %d, %v; want 3, EOF", n, err)
	}

	n, err = CopyN(ctx, WrapIoWriter(b), wantedAndErrReader{}, 5)
	if n != 5 || err != nil {
		t.Errorf("CopyN(bytes.Buffer, wantedAndErrReader, 5) = %d, %v; want 5, nil", n, err)
	}

	n, err = CopyN(ctx, &noReadFrom{WrapIoWriter(b)}, wantedAndErrReader{}, 5)
	if n != 5 || err != nil {
...
func TestReadAtLeast(t *testing.T) {
	var rb bytes.Buffer
	testReadAtLeast(t, WrapReadWriter(&rb))
}
...
func (r *dataAndErrorBuffer) Read(ctx context.Context, p []byte) (n int, err error) {
...
func (r *dataAndErrorBuffer) Write(ctx context.Context, p []byte) (n int, err error) {
...
func testReadAtLeast(t *testing.T, rb ReadWriter) {
	ctx := context.Background()
	rb.Write(ctx, []byte("0123"))
	buf := make([]byte, 2)
	n, err := ReadAtLeast(ctx, rb, buf, 2)
	if err != nil {
		t.Error(err)
	}
	if n != 2 {
	...
	n, err = ReadAtLeast(ctx, rb, buf, 4)
	...
	n, err = ReadAtLeast(ctx, rb, buf, 1)
	if err != nil {
		t.Error(err)
	}
	if n != 2 {
...
func TestTeeReader(t *testing.T) {
	...
	wb := new(bytes.Buffer)
	r := TeeReader(WrapIoReader(rb), WrapIoWriter(wb))
	if n, err := ReadFull(ctx, r, dst); err != nil || n != len(src) {
	...
	if n, err := r.Read(ctx, dst); n != 0 || err != EOF {
		t.Errorf("r.Read at EOF = %d, %v want 0, EOF", n, err)
	}
	rb = bytes.NewBuffer(src)
	pr, pw := Pipe()
	pr.Close(ctx)
	r = TeeReader(WrapIoReader(rb), pw)
	if n, err := ReadFull(ctx, r, dst); n != 0 || err != ErrClosedPipe {
...
func TestSectionReader_ReadAt(t *testing.T) {
	...
	for i, tt := range tests {
		...
		s := NewSectionReader(WrapIoReaderAt(r), int64(tt.off), int64(tt.n))
		buf := make([]byte, tt.bufLen)
		if n, err := s.ReadAt(ctx, buf, int64(tt.at)); n != len(tt.exp) || string(buf[:n]) != tt.exp || err != tt.err {
			t.Fatalf("%d: ReadAt(%d) = %q, %v; expected %q, %v", i, tt.at, buf[:n], err, tt.exp, tt.err)
		}
		if _r, off, n := s.Outer(); off != int64(tt.off) || n != int64(tt.n) {
...
func TestSectionReader_Seek(t *testing.T) {
	...
	sr := NewSectionReader(WrapIoReaderAt(br), 0, int64(len("foo")))
	...
	n, err := sr.Read(ctx, make([]byte, 10))
	if n != 0 || err != EOF {
...
func TestSectionReader_Size(t *testing.T) {
	...
	for _, tt := range tests {
		r := strings.NewReader(tt.data)
		sr := NewSectionReader(WrapIoReaderAt(r), 0, int64(len(tt.data)))
...
func TestSectionReader_Max(t *testing.T) {
	...
	sr := NewSectionReader(WrapIoReaderAt(r), 3, maxint64)
	n, err := sr.Read(ctx, make([]byte, 3))
	if n != 3 || err != nil {
	...
	if _r, off, n := sr.Outer(); off != 3 || n != maxint64 {
...
func (w largeWriter) Write(ctx context.Context, p []byte) (int, error) {
...
func TestCopyLargeWriter(t *testing.T) {
	...
	wb := largeWriter{}
	rb.WriteString("hello, world.")
	if _, err := Copy(ctx, wb, WrapIoReader(rb)); err != want {
...
func TestOffsetWriter_Seek(t *testing.T) {
	...
	defer tmpfile.Close()
	w := NewOffsetWriter(WrapIoWriterAt(tmpfile), 0)
	...
	work := func(off, at int64) {
		...
		for step, value := range []byte(content) {
			...
			go func(wg *sync.WaitGroup, tmpfile *os.File, value byte, off, at int64, step int) {
				...
				w := NewOffsetWriter(WrapIoWriterAt(tmpfile), off)
				n, e := w.WriteAt(ctx, []byte{value}, at+int64(step))
				if e != nil {
...
func TestWriteAt_PositionPriorToBase(t *testing.T) {
	...
	at := int64(-1)
	w := NewOffsetWriter(WrapIoWriterAt(tmpfile), offset)
	_, e := w.WriteAt(ctx, []byte("hello"), at)
	if e == nil {
...
func TestOffsetWriter_Write(t *testing.T) {
	...
	makeOffsetWriter := func(name string) (*OffsetWriter, *os.File) {
		...
		return NewOffsetWriter(WrapIoWriterAt(tmpfile), 0), tmpfile
	}
	...
	t.Run(name, func(t *testing.T) {
		...
		w, f := makeOffsetWriter(name)
		defer f.Close()
		Copy(ctx, w, WrapIoReader(strings.NewReader(content)))
		checkContent(name, f)
	})
}
