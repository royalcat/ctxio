...
func TestMultiReader(t *testing.T) {
	...
	withFooBar := func(tests func()) {
		...
		mr = MultiReader(WrapIoReader(r1), WrapIoReader(r2), WrapIoReader(r3))
		buf = make([]byte, 20)
		tests()
	}
	expectRead := func(size int, expected string, eerr error) {
		nread++
		n, gerr := mr.Read(ctx, buf[0:size])
		if n != len(expected) {
...
func TestMultiReaderAsWriterTo(t *testing.T) {
	ctx := context.Background()
	mr := MultiReader(
		WrapIoReader(strings.NewReader("foo ")),
		MultiReader( // Tickle the buffer reusing codepath
			WrapIoReader(strings.NewReader("")),
			WrapIoReader(strings.NewReader("bar")),
		),
	)
	...
	n, err := mrAsWriterTo.WriteTo(ctx, WrapIoWriter(sink))
	if err != nil {
...
func TestMultiWriter_WriteStringSingleAlloc(t *testing.T) {
	...
	ctx := context.Background()
	mw := MultiWriter(WrapIoWriter(&sink1), WrapIoWriter(&sink2))
	allocs := int(testing.AllocsPerRun(1000, func() {
		WriteString(ctx, mw, "foo")
	}))
	if allocs != 1 {
...
func testMultiWriter(t *testing.T, sink interface {
	...
	ctx := context.Background()
	sha1 := sha1.New()
	mw := MultiWriter(WrapIoWriter(sha1), sink)

	sourceString := "My input text."
	source := strings.NewReader(sourceString)
	written, err := Copy(ctx, mw, WrapIoReader(source))
...
type writerFunc func(ctx context.Context, p []byte) (int, error)

func (f writerFunc) Write(ctx context.Context, p []byte) (int, error) {
	return f(ctx, p)
}
...
func TestMultiWriterSingleChainFlatten(t *testing.T) {
	...
	var w Writer = MultiWriter(writerFunc(func(ctx context.Context, p []byte) (int, error) {
	...
	mw.Write(ctx, nil) // don't care about errors, just want to check the call-depth for Write
...
func TestMultiWriterError(t *testing.T) {
	...
	f1 := writerFunc(func(ctx context.Context, p []byte) (int, error) {
		return len(p) / 2, ErrShortWrite
	})
	f2 := writerFunc(func(ctx context.Context, p []byte) (int, error) {
		t.Errorf("MultiWriter called f2.Write")
		return len(p), nil
	})
	w := MultiWriter(f1, f2)
	n, err := w.Write(ctx, make([]byte, 100))
...
func TestMultiReaderCopy(t *testing.T) {
	ctx := context.Background()
	slice := []Reader{WrapIoReader(strings.NewReader("hello world"))}
	r := MultiReader(slice...)
	slice[0] = nil
...
func TestMultiWriterCopy(t *testing.T) {
	...
	slice := []Writer{WrapIoWriter(&buf)}
	w := MultiWriter(slice...)
	slice[0] = nil
	n, err := w.Write(ctx, []byte("hello world"))
	if err != nil || n != 11 {
...
type readerFunc func(ctx context.Context, p []byte) (int, error)

func (f readerFunc) Read(ctx context.Context, p []byte) (int, error) {
	return f(ctx, p)
}
...
func TestMultiReaderFlatten(t *testing.T) {
	...
	var r Reader = MultiReader(readerFunc(func(ctx context.Context, p []byte) (int, error) {
		n := runtime.Callers(1, pc)
	...
	r.Read(ctx, nil) // don't care about errors, just want to check the call-depth for Read
...
func (b byteAndEOFReader) Read(ctx context.Context, p []byte) (n int, err error) {
	if len(p) == 0 {
...
func TestMultiReaderSingleByteWithEOF(t *testing.T) {
	ctx := context.Background()
	got, err := ReadAll(ctx, LimitReader(MultiReader(byteAndEOFReader('a'), byteAndEOFReader('b')), 10))
	if err != nil {
		t.Fatal(err)
	}
	const want = "ab"
...
func TestMultiReaderFinalEOF(t *testing.T) {
	ctx := context.Background()
	r := MultiReader(WrapIoReader(bytes.NewReader(nil)), byteAndEOFReader('a'))
	buf := make([]byte, 2)
...
func TestMultiReaderFreesExhaustedReaders(t *testing.T) {
	...
	func() {
		...
		mr = MultiReader(WrapIoReader(buf1), WrapIoReader(buf2))
	...
	buf := make([]byte, 4)
	if n, err := ReadFull(ctx, mr, buf); err != nil || string(buf) != "foob" {
	...
	if n, err := ReadFull(ctx, mr, buf[:2]); err != nil || string(buf[:2]) != "ar" {
...
func TestInterleavedMultiReader(t *testing.T) {
	...
	mr1 := MultiReader(WrapIoReader(r1), WrapIoReader(r2))
	mr2 := MultiReader(mr1)
