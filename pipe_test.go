...
func checkWrite(t *testing.T, ctx context.Context, w Writer, data []byte, c chan int) {
...
func TestPipe1(t *testing.T) {
	...
	go checkWrite(t, ctx, w, []byte("hello, world"), c)
...
func reader(t *testing.T, ctx context.Context, r Reader, c chan int) {
	var buf = make([]byte, 64)
	for {
		n, err := r.Read(ctx, buf)
		if err == EOF {
...
func TestPipe3(t *testing.T) {
	...
	tot := 0
	for n := 1; n <= 256; n *= 2 {
		nn, err := r.Read(ctx, rdat[tot:tot+n])
		if err != nil && err != EOF {
			t.Fatalf("read: %v", err)
		}
...
func delayClose(t *testing.T, ctx context.Context, cl closer, ch chan int, tt pipeTest) {
...
func TestPipeReadClose(t *testing.T) {
	...
	for _, tt := range pipeTests {
		...
		r, w := Pipe()
		if tt.async {
			go delayClose(t, ctx, w, c, tt)
		} else {
			delayClose(t, ctx, w, c, tt)
		}
		var buf = make([]byte, 64)
		n, err := r.Read(ctx, buf)
		<-c
...
func TestPipeReadClose2(t *testing.T) {
	...
	c := make(chan int, 1)
	r, _ := Pipe()
	go delayClose(t, ctx, r, c, pipeTest{})
	n, err := r.Read(ctx, make([]byte, 64))
	<-c
...
func TestPipeWriteClose(t *testing.T) {
	...
	for _, tt := range pipeTests {
		...
		r, w := Pipe()
		if tt.async {
			go delayClose(t, ctx, r, c, tt)
		} else {
			delayClose(t, ctx, r, c, tt)
		}
		n, err := WriteString(ctx, w, "hello, world")
		<-c
		expect := tt.err
		if expect == nil {
...
func TestPipeWriteClose2(t *testing.T) {
	...
	c := make(chan int, 1)
	_, w := Pipe()
	go delayClose(t, ctx, w, c, pipeTest{})
	n, err := w.Write(ctx, make([]byte, 64))
	<-c
	if n != 0 || err != ErrClosedPipe {
...
func TestWriteAfterWriterClose(t *testing.T) {
	...
	go func() {
		...
		w.Close(ctx)
		_, writeErr = w.Write(ctx, []byte("world"))
		done <- true
	}()
...
func TestPipeCloseError(t *testing.T) {
	...
	if _, err := w.Write(ctx, nil); err != (testError1{}) {
	...
	if _, err := r.Read(ctx, nil); err != (testError1{}) {
	...
	t.Run("Write", func(t *testing.T) {
		...
		for i := 0; i < count; i++ {
			go func() {
				...
				if n, err := w.Write(ctx, []byte(input)); n != len(input) || err != nil {
		...
		for i := 0; i < len(buf); i += readSize {
			if n, err := r.Read(ctx, buf[i:i+readSize]); n != readSize || err != nil {
	...
	t.Run("Read", func(t *testing.T) {
		...
		for i := 0; i < cap(c); i++ {
			go func() {
				...
				if n, err := r.Read(ctx, buf); n != readSize || err != nil {
					t.Errorf("Read() = (%d, %v); want (%d, nil)", n, err, readSize)
				}
				c <- buf
			}()
